...
pub fn analyze_packet(packet_info: &mut PacketInfo) {
    ...
    let (ip_proto, src, dst) = match &sliced.ip {
        Some(InternetSlice::Ipv4(ref header, _)) => {
            ("IPv4", header.source_addr().to_string(), header.destination_addr().to_string())
        }
        Some(InternetSlice::Ipv6(ref header, _)) => {
            ("IPv6", header.source_addr().to_string(), header.destination_addr().to_string())
        }
        None => {
...
fn analyze_transport(packet_info: &mut PacketInfo, packet: &SlicedPacket) {
    match &packet.transport {
        Some(TransportSlice::Tcp(tcp)) => {
            ...
            packet_info.destination = format!("{}:{}", packet_info.destination, tcp.destination_port());
        ...
        Some(TransportSlice::Udp(udp)) => {
            ...
            packet_info.destination = format!("{}:{}", packet_info.destination, udp.destination_port());
...
mod tests {
    ...
    fn test_ipv6_packet_analysis() {
        ...
        let raw_data = vec![
            ...
            0x60, 0x00, 0x00, 0x00, 0x00, 0x20, 0x06, 0x40, // Ver, TC, Flow Label, Payload Len, Next Header (6=TCP), Hop Limit
            0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00, 0x00, 0x00,  // Source IP (2001:db8::1)
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,
            0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00, 0x00, 0x00,  // Dest IP (2001:db8::2)
